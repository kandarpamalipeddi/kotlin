...
internal class KtFirExpressionTypeProvider(
    ...
    override fun getExpectedType(expression: PsiElement): KtType? {
        ...
        val expectedType = getExpectedTypeByReturnExpression(unwrapped)
            ...
            ?: getExpectedTypeOfBlockExpression(unwrapped)
    ...
    private fun getExpectedTypeOfBlockExpression(expression: PsiElement): KtType? {
        // Given: `fun f(...): R { blockExpression }`
        // Expected type of `{ blockExpression }` is `(...) -> R`
        val block = expression as? KtBlockExpression ?: return null
        val function = block.unwrapQualified<KtFunction> { function, expr -> function.bodyExpression == expr } ?: return null
        return if (function.bodyBlockExpression != null) {
            val firAnonymousFunction = function.getOrBuildFirOfType<FirAnonymousFunctionExpression>(firResolveState).anonymousFunction
            firAnonymousFunction.typeRef.coneType.asKtType()
        } else null
    }
